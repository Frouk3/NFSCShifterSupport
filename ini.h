...
public:
	void SetIniPath(const char* filename)
	{
		...
		char* str = (char*)_malloca(len + 16);
		if (str)
			strcpy_s(str, len + 16, filename);
		...
		{
			strcpy_s(str, len + 16, "default.ini");
		}
		else
		{
			if (const char* ext = strrchr(filename, '.'); !ext && str)
				strcat_s(str, len + 16, ".ini");
		}
		...
		size_t bufferSize = strlen(buff) + strlen(str) + 2;
		m_FilePath = (char*)malloc(bufferSize);
		if (m_FilePath)
			sprintf_s(m_FilePath, bufferSize, "%s\\%s", buff, str);
	...
	// Parses INI data from a string instead of a file	
	// data param WILL be modified, make a copy if you want to keep the original data
	...
	void SetPath(const char* path)
	{
		...
		if (m_FilePath)
			strcpy_s(m_FilePath, len + 1, path);
	}
